...
`strip_duplicate_title` returns a `(body, dropped_heading_text)` tuple —
these assertions compare the body element only.
"""
...
class TestSoleH1IsATitle:
    def test_sole_leading_h1_is_removed(self):
        ...
        result, _ = strip_duplicate_title(html)
    ...
    def test_reworded_title_is_still_caught(self):
        ...
        result, _ = strip_duplicate_title(
...
class TestMultipleH1sAreSectionHeadings:
    def test_leading_h1_is_kept_when_the_document_uses_h1_for_sections(self):
        ...
        result, dropped = strip_duplicate_title(html)
        assert result == html
        assert dropped == ""
...
class TestSafety:
    ...
    def test_document_without_headings_is_untouched(self):
        html = "<p>Body only.</p>"
        result, dropped = strip_duplicate_title(html, "Some Title")
        assert result == html
        assert dropped == ""
