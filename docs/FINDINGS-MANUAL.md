...
### A title that is really a filename slug reaches the composer

**Requirement:** GRD-02

Found on 2026-08-08 while converting an unpublished drafts-folder article
whose filename was a lowercase kebab-case slug. The run exited 0 and printed
no warnings. It resolved the article's title as that filename with its
hyphens swapped for spaces — an all-lowercase string — and wrote it into
`<title>`, into `metadata.json`, onto the CLI's Title line, and onto the X11
primary selection, which is the hand-off the author middle-clicks into
Substack's title field. One gesture from a published post titled in
lowercase.

**Root cause.** `convert_article` resolves the title as the stripped leading
H1, then a frontmatter `title:`, then `source.stem.replace("-", " ")`. The
article had no frontmatter title and opened at an `##` heading, so both
earlier sources were empty and the filename fallback fired. Nothing was
broken; nothing was reported. The same silent-degradation signature as the
nine defects already in this file: the pipeline succeeds while emitting
something the author must repair by hand.

**Why the obvious check would have been useless.** Warning whenever the title
comes from the filename fires on **20 of the 25** published articles — every
one of which published fine, because those filenames were written as
sentence-cased titles, punctuation and all, and read correctly as headlines.
A check that fires on 80% of known-good output trains the author to skim past
preflight, which costs more than the defect it catches. Requiring the
resolved title to contain no uppercase letter takes it to **3 of 25**, all of
them all-lowercase filenames that read as slugs rather than headlines. That
measurement, not the rule, is the finding: a
guard's value is its ratio against real known-good output, and the ratio is
cheap to measure because the corpus is on disk.

There are also two distinct routes to the fallback, which the warning's
wording has to survive. `extract_leading_title` inherits the "leading **sole**
H1" rule, so the fallback fires both when a document has no H1 at all and
when it has a leading H1 that is not its only one — 19 of the 20 above are
the second kind, carrying between 2 and 9 H1s. Telling the author to "add an
H1" would be wrong advice for almost all of them.

**Fix:** a `slug_title` preflight check gated on both conditions, and the
fallback reported to preflight explicitly by `convert_article` — the rendered
`<title>` is identical whichever source won, so preflight cannot infer it.
The resolution order was deliberately left unchanged: the fallback is correct
and carries 20 of 25 published articles. The silence was the defect.

### Obsidian comments pasted into the post as prose

**Requirement:** GRD-02

Found in a real run: `output/okr-taxonomy-domain-alignment/article.html`
shipped the author's private notes-to-self as visible prose — one inline
aside (`<p>%% Source data for the diagram above. … %%</p>` at line 94) and a
34-line block of caption/alt-text working notes that Markdown's blank-line
handling had already split into seven separate `<p>` elements (lines
137–171) before anything downstream could see them as one unit. The
pipeline exited clean with zero warnings.

**Root cause.** There was no handler at all. `obsidian_syntax.py` had no
code for `%%`; a grep of `src/` and `tests/` for the literal returned zero
hits. The text passed untouched through `transform_obsidian_syntax()` into
python-markdown, which rendered it as ordinary prose — the same
silent-degradation signature as every other defect in this file: the
pipeline succeeds while emitting something the author must repair by hand.

**Fix:** `strip_obsidian_comments` (new, `obsidian_syntax.py`) removes both
observed shapes from the raw Markdown before rendering — the block form
carries blank lines inside it, so by the time there is HTML there is no
single element left to remove; the fix has to land before python-markdown
ever sees the text. It runs first in `transform_obsidian_syntax`, ahead of
footnote normalization, because a footnote label referenced only inside a
comment must not be collected as a live reference, and an embed or link
written inside a comment must never be turned into markup at all. A new
...
1. **Narrow scope, on purpose.** Only a same-line inline pair and a block
   ...
   stripped. A marker that opens mid-line and closes lines later is legal
   Obsidian but was not observed in the corpus and is out of scope. A
   general "opener anywhere, closer anywhere later" scanner would turn two
   unrelated stray markers in ordinary prose into the silent deletion of
   everything between them; this narrow pair of shapes fails the other way
...
2. **An unbalanced document is not stripped at all, and a doubled percent
   ...
   pass strip *nothing* for that document, and a marker preceded by a
   digit never opens a comment. The cost is that one unclosed comment
   leaves the whole document's comments in place — loudly, since every
   ...
   The digit lookbehind is narrower than it first looks, deliberately. The
   obvious guard — require whitespace before the opener — was tried and
   rejected: it silently stops stripping comments glued to a full stop, a
   blockquote `>`, an em dash, a bracket or a word, all ordinary in this
   vault's prose, and every one of those would then ship. Only the digit
   case is genuinely ambiguous, so only the digit case is refused.
3. **Pre-strip rasterization of embeds and tables inside a comment.** In
   ...
   *before* `transform_obsidian_syntax`. An image embed or a table sitting
   ...
   `table_count` / `png_files`, even though the strip removes the tag along
   with the rest of the comment before it reaches the pasted HTML. The
   result is orphan files on disk, nothing visible in the output. The call
   site is left where it is — moving it is a larger change with its own
   ordering hazards, and disk-only orphans are a materially smaller problem
   than the one this fix closes.
